...
import { postScreenshot, postText, type ResetRecord } from '@/lib/post-content';
import { announcementCopy } from '@/lib/announcements';

export function PostContent({ record, locale }: { record: ResetRecord; locale: Locale }) {
  ...
  const brief = locale === 'zh' && body.translation ? record.brief?.zh : record.brief?.en;
  const c = announcementCopy[locale];

  return <div className="post-content">
    {brief?.summary && <div className="post-brief"><strong>{c.summary}</strong><p lang={locale === 'zh' && body.translation ? 'zh-CN' : 'en'}>{brief.summary}</p>
    ...
    <details className="post-text-details">
      ...
      <div className="post-full-text">
        {locale === 'zh' && (body.translation ? <section><h4>{t('postFullTranslation')}</h4><p lang="zh-CN">{body.translation}</p></section> : <p className="post-translation-pending">{t('postTranslationPending')}</p>)}
